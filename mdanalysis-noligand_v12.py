...
# calculate radial distribution function, g(r) or sometimes noted as rdf, which looks at the spatial distribution of water molecules around residues that are most involved in dimerization. i manually picked the top residues that were most commonly in contact from contacts.dat.
interface = u.select_atoms('protein and resid 453 709 236 926 459 729 306 877')
is_water_o = u.select_atoms('(resname HOH WAT SOL) and name O')

print(f"RDF Target - Interface atoms: {len(interface)}, Water Oxygen atoms: {len(is_water_o)}")

rdf = InterRDF(interface, is_water_o, nbins=75, range=(0.0, 15.0))
rdf.run()
...
if len(bonds) > 0:
    bond_keys = bonds[:, 1:4].astype(int)
