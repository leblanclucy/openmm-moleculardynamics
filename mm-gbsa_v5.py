...
# please run mm_g-pbsa-prep.py FIRST before you run this. to do MM-GBSA or MM-PBSA, you have to take your system and separate it into three components: enzyme only, substrate only, and whole complex, all without water or ions. you then have to this to the trajectory file as well. there are also some formatting incompatibilities that will crash or throw errors if they're not fixed by running that script first (mainly with mm-pbsa which is trickier as that is done through Amber).

# 1. Load the three explicit prmtop files and the trajectory
print("Loading dedicated topologies and trajectory...")
traj = md.load('trajectory_no-solvent.nc', top='stripped_complex.prmtop')

top_complex = app.AmberPrmtopFile('stripped_complex.prmtop')
top_enzyme  = app.AmberPrmtopFile('enzyme.prmtop')
top_substrate = app.AmberPrmtopFile('substrate.prmtop')

# 2. Build OpenMM systems directly from the custom prmtop files
# This preserves the specific GLYCAM 1-4 scaling terms embedded in the files
...
# 3. Explicit numerical validation of atom counts
...
for frame_idx in range(traj.n_frames):
    ...
    # A. Minimize and evaluate full Complex
    ...
    # B. Extract positions as a raw NumPy array to avoid structural type shifts
